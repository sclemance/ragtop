#!/usr/bin/env python3
"""Generate squeekboard layouts with Esc, Tab, modifier and arrow keys, for Ragtop.

Squeekboard's layouts are GPL-3.0-or-later data files, so Ragtop doesn't
...
~/.local/state/ragtop/keyboards, which Ragtop points squeekboard at via
...
opens Ragtop's settings. The globe's layout switcher depends on GNOME
settings Omarchy doesn't use, so it does nothing there. The gear sends
XF86Tools, which Ragtop's installer binds in Hyprland to the Omarchy menu's
...
needs resolve_binds_by_sym (see Ragtop's README).
...
OUT = Path.home() / ".local/state/ragtop/keyboards"
CACHE = Path.home() / ".cache/ragtop"
...
# lets Ragtop's theme style the keys.
MOD_OUTLINE = "fl-mod"
...
def replace_globe(buttons):
    """Turn squeekboard's globe key into Ragtop's gear, in place."""
    changed = False
    for name, button in buttons.items():
...
def main():
    ...
    for path in sorted(keyboards.rglob("*.yaml")):
        ...
        dest.write_text(
            f"# Generated by Ragtop from squeekboard's {rel} (GPL-3.0-or-later).\n"
